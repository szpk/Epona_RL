...
#调用p_mean_variance（一步采样计算均值和方差），连续多步生成轨迹
    def get_action(
        self,
        vl_features: torch.Tensor,
...
#调用p_mean_variance（一步采样计算均值和方差），连续多步生成轨迹，输出完整轨迹和最终轨迹
    def sample_chain(
        self,
        vl_features: torch.Tensor,
        his_traj_features: torch.Tensor,
...
#返回的是一步采样的对数概率logpθ​(xk−1​∣xk​,s)，之后的函数调用再去做logπθ​(at​∣st​)🟰logpθ​(xk−1​∣xk​,s)连续相加
    def get_logprobs(
        self,
        vl_features: torch.Tensor,
        his_traj_features: torch.Tensor,
...
#GRPO损失函数，对最终轨迹做reward，把它按照权重作用到每一步采样的对数概率上，最终返回每个样本每条轨迹每个采样步骤的损失[N*K]，bc损失是让损失尽量跟前一步损失一致，相当于正则化效果。
    def forward_grpo(
        self,
        vl_features: torch.Tensor,
        action_input: BatchFeature,
        ...
        log_probs = self.get_logprobs(vl_features_rep, his_traj_rep, status_feature_rep, chains, deterministic=False)#[N*K, H, D]
        log_probs = log_probs.clamp(min=-5, max=2).mean(dim=[1, 2])#[N*K, H, D]->[N, K]
        
        policy_loss = -torch.mean(log_probs * adv_weighted_flat)# [N, K]->[N*K, 1]->[N*K]
