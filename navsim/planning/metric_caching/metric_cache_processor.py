...
class MetricCacheProcessor:
    ...
    def _interpolate_gt_observation(self, scenario: AbstractScenario) -> PDMObservation:
        ...
        # 计算最大可用的 iteration
        max_iteration = scenario.get_number_of_iterations() - scenario._initial_frame_idx - 1
        
        # 限制 gt_indices 不超过可用范围
        gt_indices = gt_indices[gt_indices <= max_iteration]
        
        # 调整 relative_time_s 的长度以匹配过滤后的 gt_indices
        relative_time_s = relative_time_s[:len(gt_indices)]
