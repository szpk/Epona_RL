...
# Import for coordinate transformation (same as dataset_openscene.py)
import sys
...
class NuPlan(Dataset):
    def __init__(self, data_root, json_root, cache_path=None, vae=None, split='train', condition_frames=3, block_size=1, downsample_fps=3, downsample_size=16, h=256, w=512, no_pose=False, clip_num=10000, augmenter=None, paug=0.9):
        self.split = split
        if split == 'train':
            self.meta_path = f'{json_root}/seq_save/2021.05.12.19.36.12_veh-35_00005_00204.json'
            self.pose_meta_path = f'{json_root}/ego_save'
        elif split == 'test':
            self.meta_path = f'{json_root}/seq_save/2021.05.12.19.36.12_veh-35_00005_00204.json'
            self.pose_meta_path = f'{json_root}/ego_save'
    ...
    def __getitem__(self, index):
        ...
        # Generate token for GRPO (used for metric cache lookup)
        # Format: "{seq_db_name}_{index}" to create unique identifier
        seq_data = self.sequences[index]
        seq_root = os.path.join(self.data_root, seq_data['data_root'])
        seq_db_name = os.path.basename(seq_root)
        token = f"{seq_db_name}_{index}"
        
        if self.no_pose:
            return imgs, torch.tensor(0.0), token
        else:
            return imgs, torch.stack(poses_tensor, 0).float(), token
