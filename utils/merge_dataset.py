...
class MixedBatchSampler(BatchSampler):
    ...
    def __init__(
        ...
        for i in range(len(self.raw_batches)):
            ...
            self.raw_batches_split.append(list(batch[self.rank::self.num_replicas]))

        self.n_batches = [len(b) for b in self.raw_batches_split]
        
        # IMPORTANT: Use minimum batch count across all possible ranks to ensure consistency
        # This prevents deadlock when different ranks have different batch counts
        self.n_batches_min = []
        for i in range(len(self.raw_batches)):
            batch = self.raw_batches[i]
            # Calculate batch count for all ranks and take minimum
            counts = [len(list(range(r, len(batch), self.num_replicas))) for r in range(self.num_replicas)]
            self.n_batches_min.append(min(counts))
        
        # Use minimum to ensure all ranks iterate the same number of times
        self.n_total_batch = sum(self.n_batches_min)
        # end dist
