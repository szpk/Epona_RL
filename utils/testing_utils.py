...
def plot_trajectory(predict_traj, gt_traj, video_save_path, num):
    ...
    for i in range(3):
        ...
        pred_valid = ~np.isnan(predict_traj[:, i]) & np.isfinite(predict_traj[:, i])
        gt_valid = ~np.isnan(gt_traj[:, i]) & np.isfinite(gt_traj[:, i])
        
        if np.any(pred_valid):
            axs[i].plot(range(N), predict_traj[:, i], label='predict', marker='o', markersize=4, color='blue', linewidth=2)
        if np.any(gt_valid):
            axs[i].plot(range(N), gt_traj[:, i], label='gt', marker='o', markersize=4, color='red', linewidth=2)
