...
def load_from_deepspeed_ckpt(args, model):
    # 如果同时指定了 load_from_deepspeed 和 resume_step，恢复完整训练状态（包括优化器）
    if args.load_from_deepspeed is not None and args.resume_step > 0:
        print('#### Before deepspeed resume ckpt, img_projector.0.weight sum:', torch.sum(model.module.state_dict()['model.img_projector.0.weight']))
        load_path, client_sd = model.load_checkpoint(args.load_from_deepspeed, load_module_strict=False)
        if load_path is None or client_sd is None:
            ...
            load_path, client_sd = model.load_checkpoint(resume_raw_dir, tag, load_module_strict=False)
        if client_sd is not None and 'curr_iter' in client_sd:
            args.resume_step = client_sd['curr_iter']
        print('#### After deepspeed resume ckpt, img_projector.0.weight sum:', torch.sum(model.module.state_dict()['model.img_projector.0.weight']))
    # 如果只指定了 load_from_deepspeed，只加载模型权重（用于预训练）
    elif args.load_from_deepspeed is not None:
        print('#### Before deepspeed load ckpt, img_projector.0.weight sum:', torch.sum(model.module.state_dict()['model.img_projector.0.weight']))
        ...
        print('#### After deepspeed load ckpt, img_projector.0.weight sum:', torch.sum(model.module.state_dict()['model.img_projector.0.weight']))
    # 如果只指定了 resume_step，从 save_model_path 恢复训练
        ...
        if client_sd is not None and 'curr_iter' in client_sd:
            args.resume_step = client_sd['curr_iter']
        print('#### After deepspeed resume, img_projector.0.weight sum:', torch.sum(model.module.state_dict()['model.img_projector.0.weight']))
    return model