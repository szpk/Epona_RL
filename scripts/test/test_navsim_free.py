...
def add_arguments():
    ...
    parser.add_argument('--config', default='configs/dit_config_dcae_navsim.py', type=str)
...
def test_sliding_window_img(val_data, model, args, tokenizer):
    ...
    with torch.no_grad():
        for i, (img, rot_matrix) in tqdm(enumerate(val_data)):
            ...
            print(f"Encoding time: {interval:.2f}s")
                ...
                for t2 in range(args.temporal_patch_size):
                    ...
                    with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                        predict_traj, predict_latents = model.step_eval(
                            ...
                            pose,
                            yaw,
                            self_pred_traj=True  # Use model's predicted trajectory (autoregressive)
                        )
...
def main(args):
    ...
    # Use NavSim/OpenScene dataset
    openscene_root = args.datasets_paths.get('openscene_root')
    if not openscene_root:
        raise ValueError("openscene_root must be set in config file!")

    print(f"Loading NavSim dataset from: {openscene_root}")
    test_dataset = NuPlanOpenScene(
        openscene_root=openscene_root,
        split=args.split,
        condition_frames=args.condition_frames+args.traj_len,  # For free mode, we don't need full video frames
